...
function reset(firststart = true, groups = 2, duration = 1) {
  for (const lane of lanes) {
    ...
    windowsClone.style.transform = `translateY(-${lane.clientHeight * (pool.length - 1)
      }px)`;
...
function createSlots() {
  ...
  var innerHT = '' +
    `
        <div id="slot-machine">
        <h2 id="first">Lucky Lanes Slots: Coming Soon</h2>
        <h3 id="second">Preview</h3>
          <div class="lanes">
            <div class="lane">
             <div class="windows">
                <!--blank-->
          ...
          <div class="buttons">
            <button id="button-spin">Spin</button>
            <button id="button-bet">Bet</button>
            <button id="button-exit"><strong>EXIT</strong></button>
          </div>
        </div>
      </div>
        `;
  ...
  document.querySelector('#button-exit').addEventListener('click', quitSlots);
  reset();
